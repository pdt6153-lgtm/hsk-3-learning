...
function getPracticeSentencePool(level = currentLevel) {
    ...
    const pool = [];
    getAllWords(level).forEach(word => (typeof getNaturalVocabularyExamples === 'function' ? getNaturalVocabularyExamples(word) : getVocabularyExamples(word)).forEach((example, index) => {
        if (example?.zh && example?.py && example?.vi && !/[\u3400-\u9fff]/.test(example.py) && !/ý nghĩa của|AI thêm nghĩa/i.test(example.vi)) pool.push({word, level, index, ...example});
    }));
...
function setModuleLevel(level, moduleName) {
    currentLevel = level;
...
function pickOptions(pool, correct, field='vi') {
    const other = shuffle(pool.filter(item => item[field] && item[field] !== correct[field])).slice(0,3).map(item => item[field]);
    return shuffle([correct[field], ...other]);
}
...
function setDashboardLevel(level) { currentLevel=level; renderLearningDashboard(); }
...
function renderTranslationPage(){renderModuleLevelButtons('translationLevelBtns','translation');if(!translationItem||translationItem.level!==currentLevel)nextTranslation();else drawTranslation();}
...
function drawTranslation(){const root=document.getElementById('translationArea');if(!root)return;if(!translationItem){root.innerHTML='<div class="module-result"><span>📭</span><h3>Chưa có câu dịch hợp lệ</h3></div>';return;}const source=translationDirection==='zh-vi'?translationItem.zh:translationItem.vi;root.innerHTML=`<div class="module-card translation-card"><span class="module-badge">${translationDirection==='zh-vi'?'TRUNG → VIỆT':'VIỆT → TRUNG'}</span><div class="translation-source" ${translationDirection==='zh-vi'?'lang="zh-Hans"':''}>${source}</div>${translationDirection==='zh-vi'?`<div class="translation-py">${translationItem.py}</div>`:''}<label class="sr-only" for="translationInput">Bản dịch của bạn</label><textarea id="translationInput" placeholder="Nhập bản dịch của bạn..."></textarea><div class="module-actions"><button onclick="revealTranslation()">Đối chiếu đáp án</button><button onclick="nextTranslation()">Câu khác →</button></div><div id="translationAnswer" aria-live="polite"></div></div>`;}

function revealTranslation(){const expected=translationDirection==='zh-vi'?translationItem.vi:translationItem.zh;document.getElementById('translationAnswer').innerHTML=`<div class="module-feedback correct"><strong>Đáp án tham khảo</strong><p>${expected}</p><small>Đối chiếu ý nghĩa và trật tự từ; không bắt buộc giống hoàn toàn từng chữ.</small></div>`;}
